...
const int MAXM=325000;
const int INF=1<<30;
int n;
int fir[MAXN],cur[MAXN],lev[MAXN],end[MAXM],next[MAXM],f[MAXM],ed,S,T;
int koloro[205][205],num[205][205],val[205][205],an=0;
...
int main()
{
	...
	int a,b,move[4][2]={{1,0},{0,1},{-1,0},{0,-1}},tot=0;
	...
	for(int i=1;i<=n;i++)
	{
		for(int j=1;j<=n;j++)
		{
			...
			if(koloro[i][j]==1)
			{
				addedge(S,num[i][j],val[i][j]),addedge(num[i][j],S,0);
				for(int k=0;k<4;k++)
				{
					a=i+move[k][0];
					b=j+move[k][1];
					if(1<=a&&a<=n&&1<=b&&b<=n)
						addedge(num[i][j],num[a][b],INF)
							,addedge(num[a][b],num[i][j],0);
				}
			}
			else addedge(num[i][j],T,val[i][j]),addedge(T,num[i][j],0);
		}
	}
	printf("%d",tot-Dinic());
	fclose(stdin);
